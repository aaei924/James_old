...
class PlainWikiPage {
	...
	function __construct($name, $_mysql) {
		if(!($result = $_mysql->query('SELECT `text`, `last_modify` FROM `document` WHERE `title` = "'.$_mysql->real_escape_string($name).'"'))) {
			return false;
		}
	...
	function getPage($name) {
		return new PlainWikiPage($name, $this->sql);
	}
}
class RevisionWikiPage {
	public $title, $text, $lastchanged;
	private $sql;
	function __construct($name, $_mysql,$rev) {
		if(!($result = $_mysql->query('SELECT `doc_text`, `date` FROM `revision` WHERE doc_name = "'.$name.'" AND r_num = "'.$rev.'"'))) {
			return false;
		}
		if(!($row = $result->fetch_array(MYSQLI_NUM))) {
			return false;
...
class NamuMark {
	...
	private function formatParser($line) {
		...
		for($j=0;$j<$line_len;self::nextChar($line,$j)) {
			if(self::startsWith($line, 'http', $j) && preg_match('/(https?:\/\/[^ ]+\.(jpg|jpeg|png|gif))(?:\?([^ ]+))?/i', $line, $match, 0, $j)) {
				if($this->imageAsLink)
					$innerstr = '<span class="alternative">[<a class="external" target="_blank" href="'.$match[1].'">외부 이미지</a>]</span>';
				else {
					$paramtxt = '';
					$csstxt = '';
					if(!empty($match[3])) {
	...
	private function linkProcessor($text, $type) {
		...
		include "./application/config/mark_db.php";
		$result = $mysqli->query("SELECT * FROM `document` WHERE `title` = '$href[0]'");
	...
	private function macroProcessor($text, $type) {
		...
		preg_match("/dday\((.*)\)/", $text, $matches);
		if(isset($matches[1])){
			$dday =  $matches[1];
			//return strtotime($dday);

			if(strtotime($dday) > strtotime(date('Y-m-d'))){
				return "-".(strtotime($dday) - strtotime(date('Y-m-d')))/60/24/60;
			}else if(strtotime($dday) == strtotime(date('Y-m-d'))){
				return "0";
			}else{
				return "+".(strtotime(date('Y-m-d'))-strtotime($dday) )/60/24/60;
			}
		}
		switch($macroName) {
			case 'br':
